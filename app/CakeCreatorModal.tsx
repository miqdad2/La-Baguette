...
function RemoteCake({ url }: { url: string }) {
...
function Viewer({ url }: { url: string }) {
  return <div className="creator-model"><Canvas dpr={[1, 1.6]} camera={{ position: [0, .5, 4], fov: 34 }}>
    ...
    <Suspense fallback={null}><RemoteCake url={url} /></Suspense><OrbitControls autoRotate autoRotateSpeed={.45} enableDamping enablePan={false} minDistance={2} maxDistance={8} />
  </Canvas><span>Drag to rotate in 360°</span></div>;
}
